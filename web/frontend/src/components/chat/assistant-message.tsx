...
import { AgentMarkerRenderer } from "@/components/chat/agent-markers"
  ...
  return (
    <div className="group flex w-full flex-col gap-1.5">
      ...
      <div className="bg-card text-card-foreground relative overflow-hidden rounded-xl border">
        ...
        {showDetailedSteps && meta?.active_skills && meta.active_skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 border-b border-border/50 px-4 py-2">
            {meta.active_skills.map((skill) => (
              <span
                key={skill}
                className="rounded-full bg-cyan-500/10 px-2 py-0.5 text-[11px] font-medium text-cyan-400"
              >
                {skill}
              </span>
            ))}
        ...
        {showContent && (() => {
          const hasAgentMarkers = /\[heron:/i.test(content)
          if (hasAgentMarkers) {
            return (
              <div className="max-w-none p-4 text-[15px] leading-relaxed">
                <AgentMarkerRenderer content={content} />
              </div>
            )
          }
          return (
