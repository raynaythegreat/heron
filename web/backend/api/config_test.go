...
func setupPicoEnabledEnv(t *testing.T) (string, func()) {
	...
	oldPicoHome := os.Getenv("HERON_HOME")

	if err := os.Setenv("HOME", tmp); err != nil {
		t.Fatalf("set HOME: %v", err)
	}
	if err := os.Setenv("HERON_HOME", filepath.Join(tmp, ".heron")); err != nil {
		t.Fatalf("set HERON_HOME: %v", err)
	}
	...
	cleanup := func() {
		...
		if oldPicoHome == "" {
			_ = os.Unsetenv("HERON_HOME")
		} else {
			_ = os.Setenv("HERON_HOME", oldPicoHome)
		}
	}
