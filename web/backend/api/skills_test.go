...
func TestHandleListSkills(t *testing.T) {
	...
	oldBuiltin := os.Getenv("HERON_BUILTIN_SKILLS")
	if err := os.Setenv("HERON_BUILTIN_SKILLS", builtinRoot); err != nil {
		t.Fatalf("Setenv(HERON_BUILTIN_SKILLS) error = %v", err)
	}
	defer func() {
		if oldBuiltin == "" {
			_ = os.Unsetenv("HERON_BUILTIN_SKILLS")
		} else {
			_ = os.Setenv("HERON_BUILTIN_SKILLS", oldBuiltin)
		}
	}()
