...
import (
	...
	"github.com/raynaythegreat/heron/pkg/auth/oidc"
	"github.com/raynaythegreat/heron/pkg/auth/saml"
...
func renderSSOCallbackPage(w http.ResponseWriter, flowID, status, title, errMsg string) {
	...
	_, _ = fmt.Fprintf(
		w,
		`<!doctype html><html><head><meta charset="utf-8"><title>Heron SSO</title></head><body><script>(function(){var payload=%s;var hasOpener=false;try{if(window.opener&&!window.opener.closed){window.opener.postMessage(payload,window.location.origin);hasOpener=true}}catch(e){}var target='/sso/callback?flow_id='+encodeURIComponent(payload.flowId||'')+'&status='+encodeURIComponent(payload.status||'');setTimeout(function(){if(hasOpener){window.close();return}window.location.replace(target)},800)})();</script><div style="font-family:Inter,system-ui,sans-serif;padding:24px"><h2>%s</h2><p>%s</p><p>You can close this window.</p></div></body></html>`,
		string(payloadJSON),
