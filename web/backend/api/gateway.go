...
func (h *Handler) startGatewayLocked(initialStatus string, existingPid int) (int, error) {
	...
	pid = cmd.Process.Pid
	logger.InfoC("gateway", fmt.Sprintf("Started heron gateway (PID: %d) from %s", pid, execPath))
...
// handleGatewayStart starts the heron gateway subprocess.
//
