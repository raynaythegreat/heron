...
const (
	autoStartEntryName = "HeronLauncher"
	launchAgentLabel   = "io.heron.launcher"
)
...
func linuxAutoStartPath() string {
	home, _ := os.UserHomeDir()
	return filepath.Join(home, ".config", "autostart", "heron-web.desktop")
}
...
func setLinuxAutoStart(enabled bool, exePath string, args []string) error {
	...
	if enabled {
		...
		content := strings.Join([]string{
			...
			"Version=1.0",
			"Name=Heron Web",
			"Comment=Start Heron Web on login",
