...
func TestGatewayRestartReturnsErrorStatusWhenReplacementFailsToStart(t *testing.T) {
	...
	invalidBinaryPath := filepath.Join(t.TempDir(), "fake-heron")
	if err := os.WriteFile(invalidBinaryPath, []byte("#!/bin/sh\n"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	t.Setenv("HERON_BINARY", invalidBinaryPath)
...
func TestFindPicoclawBinary_EnvOverride(t *testing.T) {
	...
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "heron-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("HERON_BINARY", mockBinary)
...
func TestFindPicoclawBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When HERON_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("HERON_BINARY", "/nonexistent/heron-binary")

	got := utils.FindPicoclawBinary()
	// Should not return the invalid path; falls back to "heron" or another found path
	if got == "/nonexistent/heron-binary" {
