...
Heron's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, Heron can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
```bash
HERON_TOOLS_EXEC_ENABLED=false
```
...
By default, Heron blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to Heron. It does **not** recursively inspect child
...
All configuration options can be overridden via environment variables with the format `HERON_TOOLS_<SECTION>_<KEY>`:

For example:

- `HERON_TOOLS_WEB_BRAVE_ENABLED=true`
- `HERON_TOOLS_EXEC_ENABLED=false`
- `HERON_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `HERON_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
- `HERON_TOOLS_MCP_ENABLED=true`
