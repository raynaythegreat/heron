...
File cấu hình: `~/.heron/config.json`

### Biến Môi Trường

Bạn có thể ghi đè các đường dẫn mặc định bằng biến môi trường. Điều này hữu ích cho cài đặt portable, triển khai container, hoặc chạy heron như dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường Dẫn Mặc Định       |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Chỉ định trực tiếp cho heron file `config.json` nào cần tải, bỏ qua tất cả vị trí khác. | `~/.heron/config.json` |
| `HERON_HOME`   | Ghi đè thư mục gốc cho dữ liệu heron. Thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.heron`             |

**Ví dụ:**

```bash
# Chạy heron với file cấu hình cụ thể
# Đường dẫn workspace sẽ được đọc từ trong file cấu hình đó
HERON_CONFIG=/etc/heron/production.json heron gateway

# Chạy heron với tất cả dữ liệu lưu tại /opt/heron
# Cấu hình sẽ được tải từ mặc định ~/.heron/config.json
# Workspace sẽ được tạo tại /opt/heron/workspace
...
Heron lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.heron/workspace`):
...
Heron chạy trong môi trường sandbox mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong workspace đã cấu hình.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.heron/workspace` | Thư mục làm việc của agent               |
...
Guard bảo mật exec chỉ kiểm tra dòng lệnh mà Heron khởi chạy trực tiếp. Nó không kiểm tra đệ quy các tiến trình con được tạo bởi công cụ phát triển được phép như `make`, `go run`, `cargo`, `npm run`, hoặc script build tùy chỉnh.
...
* Chạy Heron trong container hoặc VM nếu bạn cần cách ly mạnh hơn guard tích hợp.
...
Heron có thể thực hiện tác vụ định kỳ tự động. Tạo file `HEARTBEAT.md` trong workspace:
...
* `HERON_HEARTBEAT_ENABLED=false` để tắt
* `HERON_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
| `volcengine` | LLM (Volcengine trực tiếp)              | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> **Tính năng mới:** Heron hiện sử dụng cách tiếp cận **lấy mô hình làm trung tâm**. Chỉ cần chỉ định định dạng `vendor/model` (ví dụ: `zhipu/glm-4.7`) để thêm provider mới — **không cần thay đổi code!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Lấy](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Chỉ OAuth                                                        |

#### Cân Bằng Tải

Cấu hình nhiều endpoint cho cùng tên mô hình — Heron sẽ tự động round-robin:
...
Heron định tuyến provider theo họ giao thức:
...
Heron hỗ trợ tác vụ theo lịch qua công cụ `cron`.
...
Tác vụ đã lên lịch được lưu trữ bền vững sau khi khởi động lại tại `~/.heron/workspace/cron/`.
