...
File di configurazione: `~/.heron/config.json`

### Variabili d'Ambiente

Puoi sovrascrivere i percorsi predefiniti usando variabili d'ambiente. Questo è utile per installazioni portatili, distribuzioni containerizzate, o per eseguire heron come servizio di sistema. Queste variabili sono indipendenti e controllano percorsi diversi.

| Variabile         | Descrizione                                                                                                                             | Percorso Predefinito      |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | Sovrascrive il percorso al file di configurazione. Indica direttamente a heron quale `config.json` caricare, ignorando tutte le altre posizioni. | `~/.heron/config.json` |
| `HERON_HOME`   | Sovrascrive la directory radice per i dati di heron. Modifica la posizione predefinita del `workspace` e delle altre directory dati.  | `~/.heron`             |

**Esempi:**

```bash
# Esegui heron usando un file di configurazione specifico
# Il percorso del workspace verrà letto da quel file di configurazione
HERON_CONFIG=/etc/heron/production.json heron gateway

# Esegui heron con tutti i dati salvati in /opt/heron
# La configurazione verrà caricata dal percorso predefinito ~/.heron/config.json
# Il workspace verrà creato in /opt/heron/workspace
...
Heron salva i dati nel workspace configurato (predefinito: `~/.heron/workspace`):
...
Heron esegue in un ambiente sandboxed per impostazione predefinita. L'agent può accedere solo ai file ed eseguire comandi all'interno del workspace configurato.
...
| ----------------------- | ----------------------- | ---------------------------------------------------- |
| `workspace`             | `~/.heron/workspace` | Directory di lavoro dell'agent                       |
...
Il controllo di sicurezza exec ispeziona solo la riga di comando avviata direttamente da Heron. Non ispeziona ricorsivamente i processi figlio generati da strumenti di sviluppo consentiti come `make`, `go run`, `cargo`, `npm run` o script di build personalizzati.
...
* Esegui Heron in un container o VM se hai bisogno di un isolamento più forte di quello fornito dal controllo integrato.
...
Heron può eseguire task periodici automaticamente. Crea un file `HEARTBEAT.md` nel tuo workspace:
