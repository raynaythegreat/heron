...
Heron supports encrypting `api_key` values in `model_list` configuration entries.
...
Run `heron onboard` — it prompts for your passphrase and generates the SSH key,
...
| Encrypted | `enc://<base64>` | Decrypted at startup using `HERON_KEY_PASSPHRASE` |
...
1. The **passphrase** (`HERON_KEY_PASSPHRASE`)
...
|----------|----------|-------------|
| `HERON_KEY_PASSPHRASE` | Yes (for `enc://`) | Passphrase used for key derivation |
| `HERON_SSH_KEY_PATH` | No | Path to SSH private key. If not set, auto-detects from `~/.ssh/heron_ed25519.key` |

### SSH Key Auto-Detection

If `HERON_SSH_KEY_PATH` is not set, Heron looks for the heron-specific key:
...
Run `heron onboard` to generate it automatically.

`os.UserHomeDir()` is used for cross-platform home directory resolution (reads `USERPROFILE` on Windows, `HOME` on Unix/macOS).

> **Note:** An SSH key file is required for credential encryption. If no key is found and `HERON_SSH_KEY_PATH` is not set, encryption/decryption will fail. Run `heron onboard` to generate the key automatically.

---

## Migration

Because the only secret material is `HERON_KEY_PASSPHRASE` and the SSH private key file, migration is straightforward:

1. Copy the config file to the new machine.
2. Set `HERON_KEY_PASSPHRASE` to the same value.
3. Copy the SSH private key file to the same path (or set `HERON_SSH_KEY_PATH` to its new location).
...
- **Both passphrase and SSH key are required.** The SSH key acts as a second factor — without it, encryption/decryption will fail. Run `heron onboard` to generate the key if it doesn't exist.
- **The SSH key is read-only at runtime.** Heron never writes to or modifies the SSH key file.
- **Plaintext keys remain supported.** Existing configs without `enc://` are unaffected.
- **The `enc://` format is versioned** via the HKDF `info` field (`heron-credential-v1`), allowing future algorithm upgrades without breaking existing encrypted values.
