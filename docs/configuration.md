...
Config file: `~/.heron/config.json`

### Environment Variables

You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running heron as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | Overrides the path to the configuration file. This directly tells heron which `config.json` to load, ignoring all other locations. | `~/.heron/config.json` |
| `HERON_HOME`   | Overrides the root directory for heron data. This changes the default location of the `workspace` and other data directories.          | `~/.heron`             |

> **Note**: Legacy environment variables `HERON_CONFIG` and `HERON_HOME` are still supported for backward compatibility.

**Examples:**

```bash
# Run heron using a specific config file
# The workspace path will be read from within that config file
HERON_CONFIG=/etc/heron/production.json heron gateway

# Run heron with all its data stored in /opt/heron
# Config will be loaded from the default ~/.heron/config.json
# Workspace will be created at /opt/heron/workspace
HERON_HOME=/opt/heron heron agent

# Use both for a fully customized setup
HERON_HOME=/srv/heron HERON_CONFIG=/srv/heron/main.json heron gateway
```

### Workspace Layout

Heron stores data in your configured workspace (default: `~/.heron/workspace`):
...
1. `~/.heron/workspace/skills` (workspace)
2. `~/.heron/skills` (global)
...
```bash
export HERON_BUILTIN_SKILLS=/path/to/skills
```
...
When multiple bindings exist, Heron resolves in this order:
...
If a binding points to a missing `agent_id`, Heron falls back to the default agent.

#### How matching works (step-by-step)

1. Heron first filters bindings by `match.channel` (must equal current channel).
...
Heron runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.heron/workspace` | Working directory for the agent           |
...
The exec safety guard only inspects the command line Heron launches directly. It does not recursively inspect child
...
* Run Heron inside a container or VM if you need stronger isolation than the built-in guard provides.
...
```bash
export HERON_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
Heron can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
* `HERON_HEARTBEAT_ENABLED=false` to disable
* `HERON_HEARTBEAT_INTERVAL=60` to change interval
...
| `volcengine` | LLM (Volcengine direct)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> **What's New?** Heron now uses a **model-centric** configuration approach. Simply specify `vendor/model` format (e.g., `zhipu/glm-4.7`) to add new providers — **zero code changes required!**
...
Heron supports separating sensitive data (API keys, tokens, secrets) from your main configuration by storing them in a `.security.yml` file.
...
1. Create `~/.heron/.security.yml` with your API keys:
```yaml
model_list:
  gpt-5.4:
    api_keys:
...
| **VolcEngine (Doubao)** | `volcengine/`     | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Get Key](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> Run `heron auth login --provider anthropic` to paste your API token.
...
Heron strips only the outer `litellm/` prefix before sending the request, so `litellm/lite-gpt4` sends `lite-gpt4`, while `litellm/openai/gpt-4o` sends `openai/gpt-4o`.

</details>

#### Load Balancing

Configure multiple endpoints for the same model name — Heron will automatically round-robin between them:
...
Heron routes providers by protocol family:
...
Heron supports cron-style scheduled tasks via the `cron` tool. The agent can set, list, and cancel reminders or recurring jobs that trigger at specified times.
...
Scheduled tasks persist across restarts and are stored in `~/.heron/workspace/cron/`.
