# Using Antigravity Provider in Heron

This guide explains how to set up and use the **Antigravity** (Google Cloud Code Assist) provider in Heron.
...
6.  **Paste it back into the terminal** where Heron is waiting.

Heron will extract the authorization code and complete the process automatically.
...
### Switch Models
You can change the default model in `~/.heron/config.json` or override it via the CLI:
...
*   **429 Rate Limit**: Antigravity has strict quotas. Heron will display the "reset time" in the error message if you hit a limit.
*   **404 Not Found**: Ensure you are using a model ID from the `heron auth models` list. Use the short ID (e.g., `gemini-3-flash`) not the full path.
