...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no Heron.
...
10. [Criando um Novo Provedor no Heron](#criando-um-novo-provedor-no-heron)
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<HeronConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- Base de código do Heron (`pkg/providers/` e `pkg/auth/`)
...
Os perfis de autenticação são armazenados em `~/.heron/auth.json`:
...
## Criando um Novo Provedor no Heron

Os provedores do Heron são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/heron/internal/auth/helpers.go`:
...
heron auth login --provider your-provider
...
heron agent -m "Hello" --model your-model
```
...
export HERON_AGENTS_DEFAULTS_MODEL=your-model

# Substituir configurações do provedor
export HERON_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.heron/auth.json`)
  ...
  - `cmd/heron/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `heron auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.heron/auth.json`
- Executar novamente `heron auth login --provider antigravity`
