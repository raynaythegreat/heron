...
# Usando o provedor Antigravity no Heron

Este guia explica como configurar e usar o provedor **Antigravity** (Google Cloud Code Assist) no Heron.
...
6.  **Cole-a de volta no terminal** onde o Heron está aguardando.

O Heron extrairá automaticamente o código de autorização e completará o processo.
...
### Trocar de modelo
Você pode alterar o modelo padrão em `~/.heron/config.json` ou substituí-lo via CLI:
...
*   **429 Limite de taxa**: O Antigravity possui cotas rigorosas. O Heron exibirá o "tempo de redefinição" na mensagem de erro se você atingir um limite.
*   **404 Não encontrado**: Certifique-se de que está usando um ID de modelo da lista `heron auth models`. Use o ID curto (ex.: `gemini-3-flash`) e não o caminho completo.
