...
Converse com seu heron através do Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot ou MaixCam
...
| **Pico**             | ⭐ Fácil           | Canal de protocolo nativo Heron                    |                                                                                                                  |
...
O Heron agora mantém definições de comandos em um registro compartilhado. Na inicialização, o Telegram registrará automaticamente os comandos de bot suportados (por exemplo `/start`, `/help`, `/show`, `/list`) para que o menu de comandos e o comportamento em tempo de execução permaneçam sincronizados.
...
Se o registro de comandos falhar (erros transitórios de rede/API), o canal ainda inicia e o Heron tenta novamente o registro em segundo plano.
...
O Heron pode se conectar ao WhatsApp de duas formas:
...
Se `session_store_path` estiver vazio, a sessão é armazenada em `<workspace>/whatsapp/`. Execute `heron gateway`; na primeira execução, escaneie o QR code impresso no terminal com WhatsApp → Dispositivos Vinculados.
...
O Heron suporta conexão com sua conta pessoal do WeChat usando a API oficial Tencent iLink.
...
4. Execute `heron gateway` e abra o QQ para conversar com seu bot
...
* Configure conforme mostrado acima e execute `heron gateway`
...
O Heron suporta três tipos de integração WeCom:
...
O Heron se conecta ao Feishu via modo WebSocket/SDK — não é necessário URL de webhook público nem servidor de callback.
...
OneBot é um protocolo aberto para bots QQ. O Heron se conecta a qualquer implementação compatível com OneBot v11 (ex.: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
