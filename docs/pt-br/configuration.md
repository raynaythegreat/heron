...
Arquivo de configuração: `~/.heron/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou execução do heron como serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso indica diretamente ao heron qual `config.json` carregar, ignorando todos os outros locais. | `~/.heron/config.json` |
| `HERON_HOME`   | Substitui o diretório raiz para dados do heron. Isso altera o local padrão do `workspace` e outros diretórios de dados.          | `~/.heron`             |

**Exemplos:**

```bash
# Executar heron usando um arquivo de configuração específico
...
# Executar heron com todos os dados armazenados em /opt/heron
# A configuração será carregada do padrão ~/.heron/config.json
# O workspace será criado em /opt/heron/workspace
...
O Heron armazena dados no seu workspace configurado (padrão: `~/.heron/workspace`):
...
O Heron é executado em um ambiente sandbox por padrão. O agente só pode acessar arquivos e executar comandos dentro do workspace configurado.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.heron/workspace` | Diretório de trabalho do agente           |
...
O guard de segurança do exec inspeciona apenas a linha de comando que o Heron executa diretamente. Ele não inspeciona recursivamente processos filhos gerados por ferramentas de desenvolvimento permitidas como `make`, `go run`, `cargo`, `npm run` ou scripts de build personalizados.
...
* Execute o Heron dentro de um contêiner ou VM se precisar de isolamento mais forte do que o guard integrado oferece.
...
O Heron pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `HERON_HEARTBEAT_ENABLED=false` para desativar
* `HERON_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
| `volcengine` | LLM (Volcengine direto)                 | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> **Novidade:** Heron agora usa uma abordagem **centrada no modelo**. Basta especificar o formato `vendor/model` (ex.: `zhipu/glm-4.7`) para adicionar novos providers — **sem alterações de código!**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obter](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | Somente OAuth                                                    |

#### Balanceamento de Carga

Configure múltiplos endpoints para o mesmo nome de modelo — Heron fará round-robin automaticamente:
...
Heron roteia providers por família de protocolo:
...
Heron suporta tarefas agendadas via ferramenta `cron`.
...
As tarefas agendadas persistem após reinicializações em `~/.heron/workspace/cron/`.
