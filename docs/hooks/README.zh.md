...
如果你是在开发 Heron 本体，而不是只想验证协议，那么再看后面的 Go in-process 示例。
...
注意：`HERON_HOOK_LOG_FILE` 没有默认值。不设置时，脚本不会自动落盘日志。
...
当前宿主不会接受 process hook 主动发起的新 RPC。也就是说，外部 hook 现在只能“响应 Heron 的调用”，不能反向调用宿主去发送 channel 消息。
