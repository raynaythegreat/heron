  ...
  "hooks": {
    ...
    "processes": {
      "py_review_gate": {
        ...
        "env": {
          "HERON_HOOK_LOG_FILE": "/tmp/heron-hook-review-gate.log"
        }
      }
    }
  }
...
```bash
tail -f /tmp/heron-hook-review-gate.log
```

If you are developing Heron itself rather than only validating the protocol, continue with the Go in-process example as well.
...
- Python process example
  Best for understanding the `JSON-RPC over stdio` protocol and verifying the message flow between Heron and an external process
...
hook := myhooks.NewExampleLoggerHook(myhooks.ExampleLoggerHookOptions{
    LogFile:   "/tmp/heron-hook-example-logger.log",
    LogEvents: true,
})
  ...
  "hooks": {
    ...
    "builtins": {
      "example_logger": {
        ...
        "priority": 10,
        "config": {
          "log_file": "/tmp/heron-hook-example-logger.log",
          "log_events": true
        }
      }
    }
...
LOG_EVENTS = os.getenv("HERON_HOOK_LOG_EVENTS", "1").lower() not in {"0", "false", "no"}
LOG_FILE = os.getenv("HERON_HOOK_LOG_FILE", "").strip()
...
Note: `HERON_HOOK_LOG_FILE` has no default. If you do not set it, the script does not write any file logs.
...
- Heron starts the external process
...
The host does not currently accept new RPCs initiated by the process hook. In practice, that means an external hook can only respond to Heron calls; it cannot call back into the host to send channel messages.
