...
Fichier de configuration : `~/.heron/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de Heron en tant que service système. Ces variables sont indépendantes et contrôlent des chemins différents.

| Variable          | Description                                                                                                                             | Chemin par défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | Remplace le chemin vers le fichier de configuration. Indique directement à Heron quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.heron/config.json` |
| `HERON_HOME`   | Remplace le répertoire racine des données Heron. Change l'emplacement par défaut du `workspace` et des autres répertoires de données. | `~/.heron`             |
...
Heron stocke les données dans votre workspace configuré (par défaut : `~/.heron/workspace`) :
...
Heron s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes que dans le workspace configuré.
...
| ----------------------- | ----------------------- | ------------------------------------------------- |
| `workspace`             | `~/.heron/workspace` | Répertoire de travail de l'agent                  |
...
Le garde de sécurité exec n'inspecte que la ligne de commande lancée directement par Heron. Il n'inspecte pas récursivement les processus enfants générés par les outils de développement autorisés tels que `make`, `go run`, `cargo`, `npm run` ou les scripts de build personnalisés.
...
* Exécutez Heron dans un conteneur ou une VM si vous avez besoin d'une isolation plus forte que celle fournie par le garde intégré.
...
Heron peut effectuer des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `HERON_HEARTBEAT_ENABLED=false` pour désactiver
* `HERON_HEARTBEAT_INTERVAL=60` pour changer l'intervalle
...
> **Nouveauté :** Heron utilise désormais une approche **centrée sur le modèle**. Spécifiez simplement le format `vendor/model` (ex. `zhipu/glm-4.7`) pour ajouter de nouveaux providers — **aucune modification de code requise !**
...
| **VolcEngine (Doubao)** | `volcengine/`   | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [Obtenir](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
| **Antigravity**         | `antigravity/`  | Google Cloud                                        | Custom    | OAuth uniquement                                                 |

#### Équilibrage de Charge

Configurez plusieurs endpoints pour le même nom de modèle — Heron effectuera automatiquement un round-robin :
...
Heron route les providers par famille de protocole :
...
Heron supporte les tâches planifiées via l'outil `cron`. L'agent peut définir, lister et annuler des rappels ou tâches récurrentes.
...
Les tâches planifiées persistent après redémarrage dans `~/.heron/workspace/cron/`.
