...
# Utiliser le fournisseur Antigravity dans Heron

Ce guide explique comment configurer et utiliser le fournisseur **Antigravity** (Google Cloud Code Assist) dans Heron.
...
```bash
heron auth login --provider antigravity
```

### Authentification manuelle (Headless/VPS)
Si vous exécutez Heron sur un serveur (Coolify/Docker) et ne pouvez pas accéder à `localhost`, suivez ces étapes :
...
6.  **Collez-la dans le terminal** où Heron attend.

Heron extraira automatiquement le code d'autorisation et terminera le processus.
...
### Changer de modèle
Vous pouvez modifier le modèle par défaut dans `~/.heron/config.json` ou le remplacer via le CLI :

```bash
# Remplacer pour une seule commande
heron agent -m "Hello" --model claude-opus-4-6-thinking
```
...
1.  **Variables d'environnement** :
    *   `HERON_AGENTS_DEFAULTS_MODEL=gemini-flash`
2.  **Persistance de l'authentification** :
    ...
    ```bash
    scp ~/.heron/auth.json user@your-server:~/.heron/
    ```
...
*   **429 Limite de débit** : Antigravity a des quotas stricts. Heron affichera le « temps de réinitialisation » dans le message d'erreur si vous atteignez une limite.
*   **404 Non trouvé** : Assurez-vous d'utiliser un ID de modèle provenant de la liste `heron auth models`. Utilisez l'ID court (par ex. `gemini-3-flash`) et non le chemin complet.
