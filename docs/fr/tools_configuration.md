...
La configuration des outils de Heron se trouve dans le champ `tools` de `config.json`.
...
Par défaut, Heron bloque les commandes dangereuses suivantes :
...
Le garde exec ne valide que la commande de niveau supérieur envoyée à Heron. Il n'inspecte **pas** récursivement les processus enfants générés par les outils de build ou les scripts après le démarrage de cette commande.
...
Toutes les options de configuration peuvent être remplacées via des variables d'environnement au format `HERON_TOOLS_<SECTION>_<KEY>` :
