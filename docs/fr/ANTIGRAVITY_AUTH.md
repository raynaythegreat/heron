...
10. [Créer un nouveau fournisseur dans Heron](#créer-un-nouveau-fournisseur-dans-heron)
...
- Go ≥ 1.25
- Base de code Heron (`pkg/providers/` et `pkg/auth/`)
...
Les profils d'authentification sont stockés dans `~/.heron/auth.json` :
...
## Créer un nouveau fournisseur dans Heron

Les fournisseurs Heron sont implémentés en tant que packages Go sous `pkg/providers/`. Pour ajouter un nouveau fournisseur :
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/heron/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `pkg/auth/store.go` - Stockage des identifiants d'authentification (`~/.heron/auth.json`)
  ...
  - `cmd/heron/internal/auth/helpers.go` - Commandes CLI d'authentification
...
- Rafraîchir les jetons OAuth : `heron auth login --provider antigravity`
...
- Vérifier le stockage du profil d'authentification : `~/.heron/auth.json`
- Relancer `heron auth login --provider antigravity`
