...
Heron prend en charge le chiffrement des valeurs `api_key` dans les entrées de configuration `model_list`.
...
```bash
export HERON_KEY_PASSPHRASE="your-passphrase"
```

**2. Chiffrer une clé API**

Exécutez `heron onboard` — il vous demande votre phrase secrète et génère la clé SSH,
...
| Chiffré | `enc://<base64>` | Déchiffré au démarrage avec `HERON_KEY_PASSPHRASE` |
...
aes_key = HKDF-SHA256(ikm, salt, info="heron-credential-v1", 32 bytes)
```
...
1. La **phrase secrète** (`HERON_KEY_PASSPHRASE`)
...
|----------|--------|-------------|
| `HERON_KEY_PASSPHRASE` | Oui (pour `enc://`) | Phrase secrète utilisée pour la dérivation de clé |
| `HERON_SSH_KEY_PATH` | Non | Chemin vers la clé privée SSH. Si non défini, détection automatique depuis `~/.ssh/heron_ed25519.key` |

### Détection automatique de la clé SSH

Si `HERON_SSH_KEY_PATH` n'est pas défini, Heron recherche la clé dédiée :

```
~/.ssh/heron_ed25519.key
```

Ce fichier dédié évite les conflits avec les clés SSH existantes de l'utilisateur.
Exécutez `heron onboard` pour le générer automatiquement.
...
> **Remarque :** Un fichier de clé SSH est requis pour le chiffrement des identifiants. Si aucune clé n'est trouvée et que `HERON_SSH_KEY_PATH` n'est pas défini, le chiffrement/déchiffrement échouera. Exécutez `heron onboard` pour générer la clé automatiquement.

---

## Migration

Étant donné que les seuls éléments secrets sont `HERON_KEY_PASSPHRASE` et le fichier de clé privée SSH, la migration est simple :

1. Copiez le fichier de configuration sur la nouvelle machine.
2. Définissez `HERON_KEY_PASSPHRASE` avec la même valeur.
3. Copiez le fichier de clé privée SSH au même chemin (ou définissez `HERON_SSH_KEY_PATH` vers son nouvel emplacement).
...
- **La phrase secrète et la clé SSH sont toutes deux requises.** La clé SSH agit comme un second facteur — sans elle, le chiffrement/déchiffrement échouera. Exécutez `heron onboard` pour générer la clé si elle n'existe pas.
- **La clé SSH est en lecture seule à l'exécution.** Heron n'écrit ni ne modifie jamais le fichier de clé SSH.
- **Les clés en texte clair restent prises en charge.** Les configurations existantes sans `enc://` ne sont pas affectées.
- **Le format `enc://` est versionné** via le champ `info` de HKDF (`heron-credential-v1`), permettant de futures mises à niveau d'algorithme sans casser les valeurs chiffrées existantes.
