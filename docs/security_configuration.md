...
Heron supports separating sensitive data (API keys, tokens, secrets, passwords) from the main configuration by storing them in a `.security.yml` file. This improves security by:
...
```bash
cp security.example.yml ~/.heron/.security.yml
```

### Step 2: Fill in your actual values

Edit `~/.heron/.security.yml` and replace placeholder values with your actual API keys and tokens.
...
Restart Heron and verify it loads correctly:
```bash
heron --version
```
...
**For models:**
```bash
export HERON_CHANNELS_TELEGRAM_TOKEN="token-from-env"
```

**For channels:**
```bash
export HERON_CHANNELS_TELEGRAM_TOKEN="token-from-env"
export HERON_CHANNELS_FEISHU_APP_SECRET="secret-from-env"
```

**For web tools:**
```bash
export HERON_TOOLS_WEB_BRAVE_API_KEY="key-from-env"
export HERON_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"
```

Environment variables have the highest priority and will override both `config.json` and `.security.yml` values.

The pattern is: `HERON_<SECTION>_<KEY>_<FIELD>` with underscores separating path segments and converted to uppercase.

## Security Best Practices

1. **Never commit `.security.yml`** to version control
2. **Add to .gitignore**: Ensure `.security.yml` is in your `.gitignore` file
3. **Set file permissions**: `chmod 600 ~/.heron/.security.yml`
...
- Verify the file permissions allow reading (`chmod 600 ~/.heron/.security.yml`)
...
```bash
cp ~/.heron/config.json ~/.heron/config.json.backup
```
...
Edit `~/.heron/.security.yml` and replace placeholder values with your actual keys.
...
```bash
rm ~/.heron/config.json.backup
```

## Advanced: Encrypted API Keys

Heron supports encrypting API keys in the security file for additional protection.

### Setup

1. Set a passphrase via environment variable:
```bash
export HERON_CREDENTIAL_PASSPHRASE="your-secure-passphrase"
```
