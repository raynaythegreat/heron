...
設定ファイルパス: `~/.heron/config.json`

### 環境変数

環境変数を使用してデフォルトパスを上書きできます。ポータブルインストール、コンテナ化デプロイ、または heron をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | 設定ファイルのパスを上書きします。heron がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.heron/config.json` |
| `HERON_HOME`   | heron データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.heron`             |

**例：**

```bash
# 特定の設定ファイルで heron を実行
# ワークスペースパスはその設定ファイル内から読み込まれます
HERON_CONFIG=/etc/heron/production.json heron gateway

# /opt/heron にすべてのデータを保存して heron を実行
# 設定はデフォルトの ~/.heron/config.json から読み込まれます
# ワークスペースは /opt/heron/workspace に作成されます
...
Heron は設定されたワークスペース（デフォルト: `~/.heron/workspace`）にデータを保存します：
...
1. `~/.heron/workspace/skills`（ワークスペース）
2. `~/.heron/skills`（グローバル）
...
Heron はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
| オプション              | デフォルト値            | 説明                                  |
| ----------------------- | ----------------------- | ------------------------------------- |
| `workspace`             | `~/.heron/workspace` | Agent の作業ディレクトリ              |
...
exec セキュリティガードは Heron が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で Heron を実行してください。
...
Heron は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
- `HERON_HEARTBEAT_ENABLED=false` で無効化
- `HERON_HEARTBEAT_INTERVAL=60` で間隔を変更
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> **新機能：** Heron は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、Heron が自動的にラウンドロビンします：
...
Heron はプロトコルファミリーで Provider をルーティングします：
...
Heron は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
...
スケジュールタスクは再起動後も `~/.heron/workspace/cron/` に保存されます。
