...
**Antigravity**（Google Cloud Code Assist）は、Google が提供する AI モデルプロバイダーで、Google のクラウドインフラストラクチャを通じて Claude Opus 4.6 や Gemini などのモデルへのアクセスを提供します。本ドキュメントでは、認証の仕組み、モデルの取得方法、Heron での新しいプロバイダーの実装方法について完全なガイドを提供します。
...
9. [設定](#設定)
10. [Heron での新しいプロバイダーの作成](#heron-での新しいプロバイダーの作成)
...
- Go ≥ 1.25
- Heron コードベース（`pkg/providers/` および `pkg/auth/`）
...
認証プロファイルは `~/.heron/auth.json` に保存されます：
...
## Heron での新しいプロバイダーの作成

Heron のプロバイダーは `pkg/providers/` 配下の Go パッケージとして実装されます。新しいプロバイダーを追加するには：
...
プロバイダーが OAuth や特別な認証を必要とする場合、`cmd/heron/internal/auth/helpers.go` にケースを追加します：
...
- **ソースファイル：**
  ...
  - `pkg/auth/store.go` - 認証情報ストレージ（`~/.heron/auth.json`）
  - `pkg/providers/factory.go` - プロバイダーファクトリーとプロトコルルーティング
  - `pkg/providers/types.go` - プロバイダーインターフェース定義
  - `cmd/heron/internal/auth/helpers.go` - 認証 CLI コマンド
...
- OAuth トークンを更新：`heron auth login --provider antigravity`
...
- 認証プロファイルストレージを確認：`~/.heron/auth.json`
- `heron auth login --provider antigravity` を再実行してください
