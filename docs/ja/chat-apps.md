...
Heron は複数のチャットプラットフォームをサポートしており、Agent をどこにでも接続できます。
...
Heron は統一されたコマンド定義を使用します。起動時に Telegram がサポートするコマンド（例: `/start`、`/help`、`/show`、`/list`）を Bot コマンドメニューに自動登録し、メニュー表示と実際の動作を一致させます。
...
Heron は 2 つの WhatsApp 接続方式をサポートしています：
...
`session_store_path` が空の場合、セッションは `<workspace>/whatsapp/` に保存されます。`heron gateway` を実行し、初回実行時にターミナルに表示される QR コードをスキャンしてください（WhatsApp → リンクデバイス）。
...
Heron は Tencent iLink 公式 API を使用して WeChat 個人アカウントへの接続をサポートしています。
...
4. `heron gateway` を実行し、QQ を開いてボットとチャット
...
Heron は WebSocket/SDK モードで飛書に接続します — 公開 Webhook URL やコールバックサーバーは不要です。
...
Heron は 3 種類の WeCom 統合をサポートしています：
...
OneBot は QQ ボット向けのオープンプロトコルです。Heron は OneBot v11 互換の実装（例：[Lagrange](https://github.com/LagrangeDev/Lagrange.Core)、[NapCat](https://github.com/NapNeko/NapCatQQ)）に WebSocket で接続します。
