...
配置文件路径: `~/.heron/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 heron 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `HERON_CONFIG` | 覆盖配置文件的路径。这直接告诉 heron 加载哪个 `config.json`，忽略所有其他位置。 | `~/.heron/config.json` |
| `HERON_HOME`   | 覆盖 heron 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.heron`             |
...
# 在 /opt/heron 中存储所有数据运行 heron
# 配置将从默认的 ~/.heron/config.json 加载
# 工作区将在 /opt/heron/workspace 创建
...
Heron 将数据存储在您配置的工作区中（默认：`~/.heron/workspace`）：
...
Heron 默认在沙箱环境中运行。Agent 只能访问配置的工作区内的文件和执行命令。
...
| 选项                    | 默认值                  | 描述                          |
| ----------------------- | ----------------------- | ----------------------------- |
| `workspace`             | `~/.heron/workspace` | Agent 的工作目录              |
...
exec 安全守卫仅检查 Heron 直接启动的命令行。它不会递归检查由 `make`、`go run`、`cargo`、`npm run` 或自定义构建脚本等开发工具产生的子进程。
...
* 如果需要比内置守卫更强的隔离，请在容器或虚拟机中运行 Heron。
...
Heron 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `HERON_HEARTBEAT_ENABLED=false` 禁用
- `HERON_HEARTBEAT_INTERVAL=60` 更改间隔
...
| `volcengine` | LLM（火山引擎直连）                     | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> **新特性：** Heron 现在采用**以模型为中心**的配置方式。只需指定 `vendor/model` 格式（例如 `zhipu/glm-4.7`）即可接入新提供商——**无需修改任何代码！**
...
| **火山引擎 (豆包)**     | `volcengine/`     | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [获取](https://www.volcengine.com/activity/codingplan?utm_campaign=Heron&utm_content=Heron&utm_medium=devrel&utm_source=OWO&utm_term=Heron) |
...
> 运行 `heron auth login --provider anthropic` 粘贴 API Token。
...
Heron 只剥离最外层的 `litellm/` 前缀再发送请求，因此 `litellm/lite-gpt4` 发送 `lite-gpt4`，而 `litellm/openai/gpt-4o` 发送 `openai/gpt-4o`。

</details>

#### 负载均衡

为同一模型名称配置多个端点，Heron 会自动轮询：
...
Heron 通过 `cron` 工具支持 cron 风格的定时任务。Agent 可以设置、列出和取消在指定时间触发的提醒或周期性任务。
...
定时任务在重启后持久保存，存储于 `~/.heron/workspace/cron/`。
