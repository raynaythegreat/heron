...
Heron 的工具配置位于 `config.json` 的 `tools` 字段中。
...
在将工具结果发送给 LLM 之前，Heron 可以从输出中过滤敏感值（API 密钥、令牌、密码）。这可以防止 LLM 看到自己的凭据。
...
exec 守卫仅验证发送给 Heron 的顶层命令。它**不会**递归检查该命令启动后由构建工具或脚本生成的子进程。
...
所有配置选项都可以通过格式为 `HERON_TOOLS_<SECTION>_<KEY>` 的环境变量覆盖：
