...
您也可以使用 Docker Compose 运行 Heron，无需在本地安装任何环境。
...
> [!TIP]
> **Docker 用户**: 默认情况下, Gateway 监听 `127.0.0.1`，该端口不会暴露到容器外。如果需要通过端口映射访问健康检查接口，请在环境变量中设置 `HERON_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
`launcher` 镜像包含所有三个二进制文件（`heron`、`heron-launcher`、`heron-launcher-tui`），默认启动 Web 控制台，提供基于浏览器的配置和聊天界面。
...
```bash
# 提问
docker compose -f docker/docker-compose.yml run --rm heron-agent -m "2+2 等于几？"
...
**2. 配置 (Configure)** (`~/.heron/config.json`)
...
> `request_timeout` 为可选项，单位为秒。若省略或设置为 `<= 0`，Heron 使用默认超时（120 秒）。
