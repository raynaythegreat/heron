...
Heron can filter sensitive values (API keys, tokens, secrets, passwords) from tool call results before they are sent to the LLM. This prevents the LLM from seeing its own credentials, which could otherwise leak through tool output or cause confusing behavior.
...
|----------|-------------|
| `HERON_TOOLS_FILTER_SENSITIVE_DATA` | Set to `true` or `false` to override the config value |
