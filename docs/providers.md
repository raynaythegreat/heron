...
If `voice.model_name` is not configured, Heron will continue to fall back to Groq transcription when a Groq API key is available.
...
Heron strips only the outer `litellm/` prefix before sending the request, so proxy aliases like `litellm/lite-gpt4` send `lite-gpt4`, while `litellm/openai/gpt-4o` sends `openai/gpt-4o`.
...
<div align="center">
  <img src="assets/logo.jpg" alt="Heron Meme" width="512">
</div>
