...
Talk to your heron through Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot, MaixCam, or Pico (native protocol)
...
| **Pico**             | ⭐ Easy            | Native Heron protocol channel                      |                                                                                                                  |
...
Heron now keeps command definitions in one shared registry. On startup, Telegram will automatically register supported bot commands (for example `/start`, `/help`, `/show`, `/list`, `/use`) so command menu and runtime behavior stay in sync.
...
If command registration fails (network/API transient errors), the channel still starts and Heron retries registration in the background.
...
Heron can connect to WhatsApp in two ways:
...
If `session_store_path` is empty, the session is stored in `<workspace>/whatsapp/`. Run `heron gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
Heron supports connecting to your personal WeChat account using the official Tencent iLink API.
...
4. Run `heron gateway` and open QQ to chat with your bot
...
* Configure as shown above and run `heron gateway`
...
Heron now exposes WeCom as a single AI Bot channel over WebSocket.
...
Heron connects to Feishu via WebSocket/SDK mode — no public webhook URL or callback server needed.
...
OneBot is an open protocol for QQ bots. Heron connects to any OneBot v11 compatible implementation (e.g., [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
