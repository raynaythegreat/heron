...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in Heron.
...
10. [Creating a New Provider in Heron](#creating-a-new-provider-in-heron)
...
- Go ≥ 1.25
- Heron codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.heron/auth.json`:
...
## Creating a New Provider in Heron

Heron providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/heron/internal/auth/helpers.go`:
...
### "Token expired"
- Refresh OAuth tokens: `heron auth login --provider antigravity`
...
- Check auth profile storage: `~/.heron/auth.json`
- Re-run `heron auth login --provider antigravity`
