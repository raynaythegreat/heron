...
MaixCam チャンネルにより、Heron はエッジデバイスの AI バックエンドとして機能できます：

- **スマート監視**：MaixCAM が画像フレームを送信し、Heron がビジョンモデルで分析する
- **IoT 制御**：デバイスがセンサーデータを送信し、Heron がレスポンスを調整する
- **オフライン AI**：ローカルネットワークに Heron をデプロイして低遅延推論を実現する
