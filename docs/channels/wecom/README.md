...
Heron now exposes WeCom as a single `channels.wecom` channel built on the official WeCom AI Bot WebSocket API.
...
- QR-based CLI onboarding with `heron auth wecom`
- Shared allowlist and `reasoning_channel_id` routing

> No public webhook callback URL is required for this channel. Heron opens an outbound WebSocket connection to WeCom.
...
- Heron keeps the active WeCom turn so normal replies can continue the same stream when possible.
