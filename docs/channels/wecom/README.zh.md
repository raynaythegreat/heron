...
Heron 现在将企业微信统一为一个 `channels.wecom` 渠道，并基于企业微信官方 AI Bot WebSocket 协议实现。
...
- 支持通过 `heron auth wecom` 扫码写入配置
- 支持统一白名单与 `reasoning_channel_id`

> 这个渠道不再需要公网 webhook 回调地址。Heron 会主动向企业微信发起 WebSocket 连接。
...
- Heron 会保留当前会话对应的企业微信 turn，优先继续同一个流式回复。
