...
4. Enter the Token and EncodingAESKey into the Heron config file, start the service, then return to the admin console to save (WeCom will send a verification request)
...
User sends a message
  │
  ▼
Heron immediately returns {finish: false} (Agent starts processing)
  │
  ▼
...
If the Agent takes longer than approximately 30 seconds (WeCom's maximum polling window is 6 minutes), Heron will:
...
When `welcome_message` is configured, Heron will automatically reply with it when a user opens the chat window with the AI Bot (`enter_chat` event). Leave it empty to silently ignore the event.
...
- Check Heron logs to see if a GET request from WeCom was received
