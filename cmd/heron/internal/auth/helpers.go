...
const (
	supportedProvidersMsg = "supported providers: openai, anthropic, google-antigravity, qwen, minimax"
...
func authLoginGoogleAntigravity() error {
	...
	fmt.Println("Try it: heron agent -m \"Hello world\"")

	return nil
}

func authLoginQwen() error {
	cfg := auth.QwenPortalOAuthConfig()
	...
	fmt.Println("Qwen login successful!")
	if cred.AccountID != "" {
		fmt.Printf("Account: %s\n", cred.AccountID)
	}
...
func authLoginMiniMax() error {
	cfg := auth.MiniMaxOAuthConfig()
	...
	if err = auth.SetCredential("minimax", cred); err != nil {
		return fmt.Errorf("failed to save credentials: %w", err)
	}
	...
	if err == nil {
		found := false
		for i := range appCfg.ModelList {
			if isMiniMaxModel(appCfg.ModelList[i].Model) {
				appCfg.ModelList[i].AuthMethod = "oauth"
				found = true
				break
			}
	...
	fmt.Println("MiniMax login successful!")
	if cred.AccountID != "" {
		fmt.Printf("Account: %s\n", cred.AccountID)
	}
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: heron auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: heron auth login --provider google-antigravity",
		)
	}
...
// isMiniMaxModel checks if a model string belongs to minimax provider
func isMiniMaxModel(model string) bool {
	return model == "minimax" ||
