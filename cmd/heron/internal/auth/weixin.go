...
import (
	...
	"github.com/raynaythegreat/heron/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the heron
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your heron config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
