...
CONFIG_PKG=github.com/raynaythegreat/heron/pkg/config
...
WORKSPACE_DIR?=$(HERON_HOME)/workspace
...
## build: Build the heron binary for current platform
build: generate
...
## build-launcher: Build the heron-launcher (web console) binary
build-launcher:
	@echo "Building heron-launcher for $(PLATFORM)/$(ARCH)..."
	@mkdir -p $(BUILD_DIR)
	@if [ ! -f web/backend/dist/index.html ]; then \
		echo "Building frontend..."; \
		cd web/frontend && pnpm install && pnpm build:backend; \
	fi
	@$(WEB_GO) build $(GOFLAGS) -o $(BUILD_DIR)/heron-launcher-$(PLATFORM)-$(ARCH) ./web/backend
	@ln -sf heron-launcher-$(PLATFORM)-$(ARCH) $(BUILD_DIR)/heron-launcher
	@echo "Build complete: $(BUILD_DIR)/heron-launcher"
...
## build-all: Build heron for all platforms
build-all: generate
	@echo "Building for multiple platforms..."
	@mkdir -p $(BUILD_DIR)
	...
	GOOS=linux GOARCH=arm64 $(GO) build $(GOFLAGS) -ldflags "$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-arm64 ./$(CMD_DIR)
	@$(PTY_PATCH_LOONG64)
	GOOS=linux GOARCH=loong64 $(GO) build $(GOFLAGS_NO_GOOLM) -ldflags "$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-loong64 ./$(CMD_DIR)
	GOOS=linux GOARCH=riscv64 $(GO) build $(GOFLAGS_NO_GOOLM) -ldflags "$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-riscv64 ./$(CMD_DIR)
	GOOS=linux GOARCH=mipsle GOMIPS=softfloat $(GO) build $(GOFLAGS_NO_GOOLM) -ldflags "$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-mipsle ./$(CMD_DIR)
	$(call PATCH_MIPS_FLAGS,$(BUILD_DIR)/$(BINARY_NAME)-linux-mipsle)
...
## install: Install heron to system and copy builtin skills
install: build
...
## uninstall: Remove heron from system
uninstall:
...
## uninstall-all: Remove heron and all data
uninstall-all:
	@echo "Removing workspace and skills..."
	@rm -rf $(HERON_HOME)
	@echo "Removed workspace: $(HERON_HOME)"
...
vet: generate
	@packages="$$($(GO) list $(GOFLAGS) ./...)" && \
		$(GO) vet $(GOFLAGS) $$(printf '%s\n' "$$packages" | grep -v '^github.com/raynaythegreat/heron/web/')
	@cd web/backend && $(WEB_GO) vet ./...

## test: Test Go code
test: generate
	@$(GO) test $(GOFLAGS) $$($(GO) list $(GOFLAGS) ./... | grep -v github.com/raynaythegreat/heron/web/)
	@cd web && make test
...
docker-build:
	...
	docker compose -f docker/docker-compose.yml build heron-agent heron-gateway

## docker-build-full: Build Docker image with full MCP support (Node.js 24)
docker-build-full:
	@echo "Building full-featured Docker image (Node.js 24)..."
	docker compose -f docker/docker-compose.full.yml build heron-agent heron-gateway
...
## docker-run: Run heron gateway in Docker (Alpine-based)
docker-run:
	docker compose -f docker/docker-compose.yml --profile gateway up

## docker-run-full: Run heron gateway in Docker (full-featured)
docker-run-full:
	docker compose -f docker/docker-compose.full.yml --profile gateway up

## docker-run-agent: Run heron agent in Docker (interactive, Alpine-based)
docker-run-agent:
	docker compose -f docker/docker-compose.yml run --rm heron-agent

## docker-run-agent-full: Run heron agent in Docker (interactive, full-featured)
docker-run-agent-full:
	docker compose -f docker/docker-compose.full.yml run --rm heron-agent

## docker-clean: Clean Docker images and volumes
docker-clean:
	docker compose -f docker/docker-compose.yml down -v
	docker compose -f docker/docker-compose.full.yml down -v
	docker rmi heron:latest heron:full 2>/dev/null || true


## build-macos-app: Build Heron macOS .app bundle (no terminal window)
build-macos-app:
	...
	@echo "macOS .app bundle created: $(BUILD_DIR)/Heron.app"
...
help:
	...
	@echo "  WORKSPACE_DIR           # Workspace directory (default: ~/.heron/workspace)"
