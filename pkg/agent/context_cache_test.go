...
func setupWorkspace(t *testing.T, files map[string]string) string {
	t.Helper()
	tmpDir, err := os.MkdirTemp("", "heron-test-*")
	if err != nil {
		t.Fatal(err)
	}
...
// (~/.heron/skills) invalidates the cached system prompt.
func TestGlobalSkillFileContentChange(t *testing.T) {
	...
	globalSkillPath := filepath.Join(tmpHome, ".heron", "skills", "global-skill", "SKILL.md")
...
func TestBuiltinSkillFileContentChange(t *testing.T) {
	...
	t.Setenv("HERON_BUILTIN_SKILLS", builtinRoot)
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "heron") {
...
func BenchmarkBuildMessagesWithCache(b *testing.B) {
	tmpDir, _ := os.MkdirTemp("", "heron-bench-*")
	defer os.RemoveAll(tmpDir)
