...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("HERON_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"HERON_HOOK_HELPER=1",
		"HERON_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "HERON_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("HERON_HOOK_MODE")
	eventLog := os.Getenv("HERON_HOOK_EVENT_LOG")
