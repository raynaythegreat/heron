...
func TestMatrixLocalpartMentionRegexp(t *testing.T) {
	re := localpartMentionRegexp("heron")

	cases := []struct {
		text string
		want bool
	}{
		{text: "@heron hello", want: true},
		{text: "hi @heron:matrix.org", want: true},
		{
			text: "\u6b22\u8fce\u4e00\u4e0bheron\u5c0f\u9f99\u867e",
...
func TestStripUserMention(t *testing.T) {
	userID := id.UserID("@heron:matrix.org")

	cases := []struct {
		in   string
		want string
	}{
		{in: "@heron:matrix.org hello", want: "hello"},
		{in: "@heron, hello", want: "hello"},
...
func TestIsBotMentioned(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			msg: event.MessageEventContent{
				...
				Mentions: &event.Mentions{
					UserIDs: []id.UserID{id.UserID("@heron:matrix.org")},
				},
			},
			want: true,
		},
		...
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/@heron:matrix.org">Heron</a> hello`,
			},
			want: true,
		},
		{
			...
			msg: event.MessageEventContent{
				Body:          "hello bot",
				FormattedBody: `<a href="https://matrix.to/#/%40heron%3Amatrix.org">Heron</a> hello`,
			},
			want: true,
		},
	}
...
func TestDownloadMedia_WritesResponseToTempFile(t *testing.T) {
	...
	client, err := mautrix.NewClient(server.URL, id.UserID("@heron:matrix.test"), "")
	if err != nil {
