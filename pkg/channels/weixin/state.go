...
import (
	...
	basechannels "github.com/raynaythegreat/heron/pkg/channels"
...
func heronHomeDir() string {
	if home := os.Getenv(config.EnvHome); home != "" {
		return home
	}
	userHome, _ := os.UserHomeDir()
	return filepath.Join(userHome, ".heron")
}
...
func buildWeixinSyncBufPath(cfg config.WeixinConfig) string {
	...
	return filepath.Join(heronHomeDir(), "channels", "weixin", "sync", key+".json")
}
