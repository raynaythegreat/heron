...
import (
	...
	"github.com/raynaythegreat/heron/pkg/migrate/sources/openclaw"
)
...
func (m *MigrateInstance) Run(opts Options) (*Result, error) {
	...
	fmt.Println("Migrating from Source to Heron")
	fmt.Printf("  Source:      %s\n", sourceHome)
	fmt.Printf("  Target: %s\n", targetHome)
	fmt.Println()
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Description: "convert Source config to Heron format",
			})
		}
	}
