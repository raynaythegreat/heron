...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.heron/workspace" {
		t.Errorf("expected workspace '~/.heron/workspace', got '%s'", workspace)
	}
...
func TestConvertToHeron(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToHeron("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.heron/workspace" {
		t.Errorf("expected workspace '~/.heron/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToHeronWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToHeron("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestConvertToHeronWithMatrix(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestConvertToHeronWithMatrixDisabled(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.heron/workspace" {
		t.Errorf("expected workspace '~/.heron/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
