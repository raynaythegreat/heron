...
func (c *OpenClawConfig) ConvertToHeron(sourceHome string) (*HeronConfig, []string, error) {
	cfg := &HeronConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via heron CLI",
...
func (c *HeronConfig) ToStandardConfig() *config.Config {
