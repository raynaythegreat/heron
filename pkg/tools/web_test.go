...
// with cf-mitigated: challenge triggers a retry using the honest heron User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest heron user agent
	if !strings.Contains(receivedUAs[1], "heron") {
		t.Errorf("retry request UA = %q, want it to contain 'heron'", receivedUAs[1])
	}
}
