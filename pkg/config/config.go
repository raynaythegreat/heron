...
import (
	...
	"github.com/raynaythegreat/heron/pkg"
	"github.com/raynaythegreat/heron/pkg/credential"
	"github.com/raynaythegreat/heron/pkg/fileutil"
...
type SubTurnConfig struct {
	MaxDepth              int `json:"max_depth"               env:"HERON_AGENTS_DEFAULTS_SUBTURN_MAX_DEPTH"`
	MaxConcurrent         int `json:"max_concurrent"          env:"HERON_AGENTS_DEFAULTS_SUBTURN_MAX_CONCURRENT"`
	DefaultTimeoutMinutes int `json:"default_timeout_minutes" env:"HERON_AGENTS_DEFAULTS_SUBTURN_DEFAULT_TIMEOUT_MINUTES"`
	DefaultTokenBudget    int `json:"default_token_budget"    env:"HERON_AGENTS_DEFAULTS_SUBTURN_DEFAULT_TOKEN_BUDGET"`
	ConcurrencyTimeoutSec int `json:"concurrency_timeout_sec" env:"HERON_AGENTS_DEFAULTS_SUBTURN_CONCURRENCY_TIMEOUT_SEC"`
}

type ToolFeedbackConfig struct {
	Enabled       bool `json:"enabled"         env:"HERON_AGENTS_DEFAULTS_TOOL_FEEDBACK_ENABLED"`
	MaxArgsLength int  `json:"max_args_length" env:"HERON_AGENTS_DEFAULTS_TOOL_FEEDBACK_MAX_ARGS_LENGTH"`
}
...
type AgentDefaults struct {
	Workspace                 string              `json:"workspace"                       env:"HERON_AGENTS_DEFAULTS_WORKSPACE"`
	RestrictToWorkspace       bool                `json:"restrict_to_workspace"           env:"HERON_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE"`
	AllowReadOutsideWorkspace bool                `json:"allow_read_outside_workspace"    env:"HERON_AGENTS_DEFAULTS_ALLOW_READ_OUTSIDE_WORKSPACE"`
	Provider                  string              `json:"provider"                        env:"HERON_AGENTS_DEFAULTS_PROVIDER"`
	ModelName                 string              `json:"model_name"                      env:"HERON_AGENTS_DEFAULTS_MODEL_NAME"`
	ModelFallbacks            []string            `json:"model_fallbacks,omitempty"`
	ImageModel                string              `json:"image_model,omitempty"            env:"HERON_AGENTS_DEFAULTS_IMAGE_MODEL"`
	ImageModelFallbacks       []string            `json:"image_model_fallbacks,omitempty"`
	VideoModel                string              `json:"video_model,omitempty"            env:"HERON_AGENTS_DEFAULTS_VIDEO_MODEL"`
	VideoModelFallbacks       []string            `json:"video_model_fallbacks,omitempty"`
	MaxTokens                 int                 `json:"max_tokens"                       env:"HERON_AGENTS_DEFAULTS_MAX_TOKENS"`
	ContextWindow             int                 `json:"context_window,omitempty"        env:"HERON_AGENTS_DEFAULTS_CONTEXT_WINDOW"`
	Temperature               *float64            `json:"temperature,omitempty"           env:"HERON_AGENTS_DEFAULTS_TEMPERATURE"`
	MaxToolIterations         int                 `json:"max_tool_iterations"             env:"HERON_AGENTS_DEFAULTS_MAX_TOOL_ITERATIONS"`
	SummarizeMessageThreshold int                 `json:"summarize_message_threshold"     env:"HERON_AGENTS_DEFAULTS_SUMMARIZE_MESSAGE_THRESHOLD"`
	SummarizeTokenPercent     int                 `json:"summarize_token_percent"         env:"HERON_AGENTS_DEFAULTS_SUMMARIZE_TOKEN_PERCENT"`
	MaxMediaSize              int                 `json:"max_media_size,omitempty"        env:"HERON_AGENTS_DEFAULTS_MAX_MEDIA_SIZE"`
	Routing                   *RoutingConfig      `json:"routing,omitempty"`
	SteeringMode              string              `json:"steering_mode,omitempty"         env:"HERON_AGENTS_DEFAULTS_STEERING_MODE"` // "one-at-a-time" (default) or "all"
	SubTurn                   SubTurnConfig       `json:"subturn"                                                                                      envPrefix:"HERON_AGENTS_DEFAULTS_SUBTURN_"`
...
type ChannelsConfig struct {
	...
	WeCom      WeComConfig      `json:"wecom"       envPrefix:"HERON_CHANNELS_WECOM_"`
...
type StreamingConfig struct {
	Enabled         bool `json:"enabled,omitempty"          env:"HERON_CHANNELS_TELEGRAM_STREAMING_ENABLED"`
	ThrottleSeconds int  `json:"throttle_seconds,omitempty" env:"HERON_CHANNELS_TELEGRAM_STREAMING_THROTTLE_SECONDS"`
	MinGrowthChars  int  `json:"min_growth_chars,omitempty" env:"HERON_CHANNELS_TELEGRAM_STREAMING_MIN_GROWTH_CHARS"`
}

type WhatsAppConfig struct {
	Enabled            bool                `json:"enabled"              env:"HERON_CHANNELS_WHATSAPP_ENABLED"`
	BridgeURL          string              `json:"bridge_url"           env:"HERON_CHANNELS_WHATSAPP_BRIDGE_URL"`
	UseNative          bool                `json:"use_native"           env:"HERON_CHANNELS_WHATSAPP_USE_NATIVE"`
	SessionStorePath   string              `json:"session_store_path"   env:"HERON_CHANNELS_WHATSAPP_SESSION_STORE_PATH"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"           env:"HERON_CHANNELS_WHATSAPP_ALLOW_FROM"`
	ReasoningChannelID string              `json:"reasoning_channel_id" env:"HERON_CHANNELS_WHATSAPP_REASONING_CHANNEL_ID"`
}

type TelegramConfig struct {
	Enabled            bool `json:"enabled"                 env:"HERON_CHANNELS_TELEGRAM_ENABLED"`
	token              string
	BaseURL            string              `json:"base_url"                env:"HERON_CHANNELS_TELEGRAM_BASE_URL"`
	Proxy              string              `json:"proxy"                   env:"HERON_CHANNELS_TELEGRAM_PROXY"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_TELEGRAM_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Typing             TypingConfig        `json:"typing,omitempty"`
	Placeholder        PlaceholderConfig   `json:"placeholder,omitempty"`
	Streaming          StreamingConfig     `json:"streaming,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_TELEGRAM_REASONING_CHANNEL_ID"`
	UseMarkdownV2      bool                `json:"use_markdown_v2"         env:"HERON_CHANNELS_TELEGRAM_USE_MARKDOWN_V2"`
	secDirty           bool
}
...
type FeishuConfig struct {
	Enabled             bool   `json:"enabled"                 env:"HERON_CHANNELS_FEISHU_ENABLED"`
	AppID               string `json:"app_id"                  env:"HERON_CHANNELS_FEISHU_APP_ID"`
	appSecret           string
	encryptKey          string
	verificationToken   string
	AllowFrom           FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_FEISHU_ALLOW_FROM"`
	GroupTrigger        GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Placeholder         PlaceholderConfig   `json:"placeholder,omitempty"`
	ReasoningChannelID  string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_FEISHU_REASONING_CHANNEL_ID"`
	RandomReactionEmoji FlexibleStringSlice `json:"random_reaction_emoji"   env:"HERON_CHANNELS_FEISHU_RANDOM_REACTION_EMOJI"`
	IsLark              bool                `json:"is_lark"                 env:"HERON_CHANNELS_FEISHU_IS_LARK"`
	secDirty            bool
}
...
type DiscordConfig struct {
	Enabled            bool `json:"enabled"                 env:"HERON_CHANNELS_DISCORD_ENABLED"`
	token              string
	Proxy              string              `json:"proxy"                   env:"HERON_CHANNELS_DISCORD_PROXY"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_DISCORD_ALLOW_FROM"`
	MentionOnly        bool                `json:"mention_only"            env:"HERON_CHANNELS_DISCORD_MENTION_ONLY"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Typing             TypingConfig        `json:"typing,omitempty"`
	Placeholder        PlaceholderConfig   `json:"placeholder,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_DISCORD_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type MaixCamConfig struct {
	Enabled            bool                `json:"enabled"              env:"HERON_CHANNELS_MAIXCAM_ENABLED"`
	Host               string              `json:"host"                 env:"HERON_CHANNELS_MAIXCAM_HOST"`
	Port               int                 `json:"port"                 env:"HERON_CHANNELS_MAIXCAM_PORT"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"           env:"HERON_CHANNELS_MAIXCAM_ALLOW_FROM"`
	ReasoningChannelID string              `json:"reasoning_channel_id" env:"HERON_CHANNELS_MAIXCAM_REASONING_CHANNEL_ID"`
}

type QQConfig struct {
	Enabled              bool   `json:"enabled"                  env:"HERON_CHANNELS_QQ_ENABLED"`
	AppID                string `json:"app_id"                   env:"HERON_CHANNELS_QQ_APP_ID"`
	appSecret            string
	AllowFrom            FlexibleStringSlice `json:"allow_from"               env:"HERON_CHANNELS_QQ_ALLOW_FROM"`
	GroupTrigger         GroupTriggerConfig  `json:"group_trigger,omitempty"`
	MaxMessageLength     int                 `json:"max_message_length"       env:"HERON_CHANNELS_QQ_MAX_MESSAGE_LENGTH"`
	MaxBase64FileSizeMiB int64               `json:"max_base64_file_size_mib" env:"HERON_CHANNELS_QQ_MAX_BASE64_FILE_SIZE_MIB"`
	SendMarkdown         bool                `json:"send_markdown"            env:"HERON_CHANNELS_QQ_SEND_MARKDOWN"`
	ReasoningChannelID   string              `json:"reasoning_channel_id"     env:"HERON_CHANNELS_QQ_REASONING_CHANNEL_ID"`
	secDirty             bool
}
...
type DingTalkConfig struct {
	Enabled            bool   `json:"enabled"                 env:"HERON_CHANNELS_DINGTALK_ENABLED"`
	ClientID           string `json:"client_id"               env:"HERON_CHANNELS_DINGTALK_CLIENT_ID"`
	clientSecret       string
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_DINGTALK_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_DINGTALK_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type SlackConfig struct {
	Enabled            bool `json:"enabled"                 env:"HERON_CHANNELS_SLACK_ENABLED"`
	botToken           string
	appToken           string
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_SLACK_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Typing             TypingConfig        `json:"typing,omitempty"`
	Placeholder        PlaceholderConfig   `json:"placeholder,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_SLACK_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type MatrixConfig struct {
	Enabled            bool   `json:"enabled"                        env:"HERON_CHANNELS_MATRIX_ENABLED"`
	Homeserver         string `json:"homeserver"                     env:"HERON_CHANNELS_MATRIX_HOMESERVER"`
	UserID             string `json:"user_id"                        env:"HERON_CHANNELS_MATRIX_USER_ID"`
	accessToken        string
	DeviceID           string              `json:"device_id,omitempty"            env:"HERON_CHANNELS_MATRIX_DEVICE_ID"`
	JoinOnInvite       bool                `json:"join_on_invite"                 env:"HERON_CHANNELS_MATRIX_JOIN_ON_INVITE"`
	MessageFormat      string              `json:"message_format,omitempty"       env:"HERON_CHANNELS_MATRIX_MESSAGE_FORMAT"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"                     env:"HERON_CHANNELS_MATRIX_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Placeholder        PlaceholderConfig   `json:"placeholder,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"           env:"HERON_CHANNELS_MATRIX_REASONING_CHANNEL_ID"`
	secDirty           bool
	CryptoDatabasePath string `json:"crypto_database_path,omitempty" env:"HERON_CHANNELS_MATRIX_CRYPTO_DATABASE_PATH"`
	CryptoPassphrase   string `json:"crypto_passphrase,omitempty"    env:"HERON_CHANNELS_MATRIX_CRYPTO_PASSPHRASE"`
}
...
type LINEConfig struct {
	Enabled            bool `json:"enabled"                 env:"HERON_CHANNELS_LINE_ENABLED"`
	channelSecret      string
	channelAccessToken string
	WebhookHost        string              `json:"webhook_host"            env:"HERON_CHANNELS_LINE_WEBHOOK_HOST"`
	WebhookPort        int                 `json:"webhook_port"            env:"HERON_CHANNELS_LINE_WEBHOOK_PORT"`
	WebhookPath        string              `json:"webhook_path"            env:"HERON_CHANNELS_LINE_WEBHOOK_PATH"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_LINE_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Typing             TypingConfig        `json:"typing,omitempty"`
	Placeholder        PlaceholderConfig   `json:"placeholder,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_LINE_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type OneBotConfig struct {
	Enabled            bool   `json:"enabled"                 env:"HERON_CHANNELS_ONEBOT_ENABLED"`
	WSUrl              string `json:"ws_url"                  env:"HERON_CHANNELS_ONEBOT_WS_URL"`
	accessToken        string
	ReconnectInterval  int                 `json:"reconnect_interval"      env:"HERON_CHANNELS_ONEBOT_RECONNECT_INTERVAL"`
	GroupTriggerPrefix []string            `json:"group_trigger_prefix"    env:"HERON_CHANNELS_ONEBOT_GROUP_TRIGGER_PREFIX"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_ONEBOT_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Typing             TypingConfig        `json:"typing,omitempty"`
	Placeholder        PlaceholderConfig   `json:"placeholder,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_ONEBOT_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type WeixinConfig struct {
	Enabled            bool `json:"enabled"              env:"HERON_CHANNELS_WEIXIN_ENABLED"`
	token              string
	AccountID          string              `json:"account_id,omitempty" env:"HERON_CHANNELS_WEIXIN_ACCOUNT_ID"`
	BaseURL            string              `json:"base_url"             env:"HERON_CHANNELS_WEIXIN_BASE_URL"`
	CDNBaseURL         string              `json:"cdn_base_url"         env:"HERON_CHANNELS_WEIXIN_CDN_BASE_URL"`
	Proxy              string              `json:"proxy"                env:"HERON_CHANNELS_WEIXIN_PROXY"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"           env:"HERON_CHANNELS_WEIXIN_ALLOW_FROM"`
	ReasoningChannelID string              `json:"reasoning_channel_id" env:"HERON_CHANNELS_WEIXIN_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type PicoConfig struct {
	Enabled         bool `json:"enabled"                     env:"HERON_CHANNELS_PICO_ENABLED"`
	token           string
	...
	AllowFrom       FlexibleStringSlice `json:"allow_from"                  env:"HERON_CHANNELS_PICO_ALLOW_FROM"`
	Placeholder     PlaceholderConfig   `json:"placeholder,omitempty"`
	secDirty        bool
}
...
type PicoClientConfig struct {
	Enabled      bool                `json:"enabled"                 env:"HERON_CHANNELS_PICO_CLIENT_ENABLED"`
	URL          string              `json:"url"                     env:"HERON_CHANNELS_PICO_CLIENT_URL"`
	Token        string              `json:"token"                   env:"HERON_CHANNELS_PICO_CLIENT_TOKEN"`
	SessionID    string              `json:"session_id,omitempty"`
	PingInterval int                 `json:"ping_interval,omitempty"`
	ReadTimeout  int                 `json:"read_timeout,omitempty"`
	AllowFrom    FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_PICO_CLIENT_ALLOW_FROM"`
}

type IRCConfig struct {
	Enabled            bool   `json:"enabled"                 env:"HERON_CHANNELS_IRC_ENABLED"`
	Server             string `json:"server"                  env:"HERON_CHANNELS_IRC_SERVER"`
	TLS                bool   `json:"tls"                     env:"HERON_CHANNELS_IRC_TLS"`
	Nick               string `json:"nick"                    env:"HERON_CHANNELS_IRC_NICK"`
	User               string `json:"user,omitempty"          env:"HERON_CHANNELS_IRC_USER"`
	RealName           string `json:"real_name,omitempty"     env:"HERON_CHANNELS_IRC_REAL_NAME"`
	password           string
	nickServPassword   string
	SASLUser           string `json:"sasl_user"               env:"HERON_CHANNELS_IRC_SASL_USER"`
	saslPassword       string
	Channels           FlexibleStringSlice `json:"channels"                env:"HERON_CHANNELS_IRC_CHANNELS"`
	RequestCaps        FlexibleStringSlice `json:"request_caps,omitempty"  env:"HERON_CHANNELS_IRC_REQUEST_CAPS"`
	AllowFrom          FlexibleStringSlice `json:"allow_from"              env:"HERON_CHANNELS_IRC_ALLOW_FROM"`
	GroupTrigger       GroupTriggerConfig  `json:"group_trigger,omitempty"`
	Typing             TypingConfig        `json:"typing,omitempty"`
	ReasoningChannelID string              `json:"reasoning_channel_id"    env:"HERON_CHANNELS_IRC_REASONING_CHANNEL_ID"`
	secDirty           bool
}
...
type HeartbeatConfig struct {
	Enabled  bool `json:"enabled"  env:"HERON_HEARTBEAT_ENABLED"`
	Interval int  `json:"interval" env:"HERON_HEARTBEAT_INTERVAL"` // minutes, min 5
}

type DevicesConfig struct {
	Enabled    bool `json:"enabled"     env:"HERON_DEVICES_ENABLED"`
	MonitorUSB bool `json:"monitor_usb" env:"HERON_DEVICES_MONITOR_USB"`
}

type VoiceConfig struct {
	ModelName         string `json:"model_name,omitempty"         env:"HERON_VOICE_MODEL_NAME"`
	EchoTranscription bool   `json:"echo_transcription"           env:"HERON_VOICE_ECHO_TRANSCRIPTION"`
	ElevenLabsAPIKey  string `json:"elevenlabs_api_key,omitempty" env:"HERON_VOICE_ELEVENLABS_API_KEY"`
}
...
type GatewayConfig struct {
	Host      string `json:"host"                env:"HERON_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"HERON_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"HERON_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"HERON_LOG_LEVEL"`
}

type ToolDiscoveryConfig struct {
	Enabled          bool `json:"enabled"            env:"HERON_TOOLS_DISCOVERY_ENABLED"`
	TTL              int  `json:"ttl"                env:"HERON_TOOLS_DISCOVERY_TTL"`
	MaxSearchResults int  `json:"max_search_results" env:"HERON_MAX_SEARCH_RESULTS"`
	UseBM25          bool `json:"use_bm25"           env:"HERON_TOOLS_DISCOVERY_USE_BM25"`
	UseRegex         bool `json:"use_regex"          env:"HERON_TOOLS_DISCOVERY_USE_REGEX"`
}
...
type BraveConfig struct {
	Enabled    bool `json:"enabled"     env:"HERON_TOOLS_WEB_BRAVE_ENABLED"`
	apiKeys    []string
	secDirty   bool
	MaxResults int `json:"max_results" env:"HERON_TOOLS_WEB_BRAVE_MAX_RESULTS"`
}
...
type TavilyConfig struct {
	Enabled    bool `json:"enabled"     env:"HERON_TOOLS_WEB_TAVILY_ENABLED"`
	apiKeys    []string
	secDirty   bool
	BaseURL    string `json:"base_url"    env:"HERON_TOOLS_WEB_TAVILY_BASE_URL"`
	MaxResults int    `json:"max_results" env:"HERON_TOOLS_WEB_TAVILY_MAX_RESULTS"`
}
...
type DuckDuckGoConfig struct {
	Enabled    bool `json:"enabled"     env:"HERON_TOOLS_WEB_DUCKDUCKGO_ENABLED"`
	MaxResults int  `json:"max_results" env:"HERON_TOOLS_WEB_DUCKDUCKGO_MAX_RESULTS"`
}

type PerplexityConfig struct {
	Enabled    bool `json:"enabled"     env:"HERON_TOOLS_WEB_PERPLEXITY_ENABLED"`
	apiKeys    []string
	secDirty   bool
	MaxResults int `json:"max_results" env:"HERON_TOOLS_WEB_PERPLEXITY_MAX_RESULTS"`
}
...
type SearXNGConfig struct {
	Enabled    bool   `json:"enabled"     env:"HERON_TOOLS_WEB_SEARXNG_ENABLED"`
	BaseURL    string `json:"base_url"    env:"HERON_TOOLS_WEB_SEARXNG_BASE_URL"`
	MaxResults int    `json:"max_results" env:"HERON_TOOLS_WEB_SEARXNG_MAX_RESULTS"`
}

type GLMSearchConfig struct {
	Enabled  bool `json:"enabled"  env:"HERON_TOOLS_WEB_GLM_ENABLED"`
	apiKey   string
	secDirty bool
	BaseURL  string `json:"base_url" env:"HERON_TOOLS_WEB_GLM_BASE_URL"`
	// SearchEngine specifies the search backend: "search_std" (default),
	// "search_pro", "search_pro_sogou", or "search_pro_quark".
	SearchEngine string `json:"search_engine" env:"HERON_TOOLS_WEB_GLM_SEARCH_ENGINE"`
	MaxResults   int    `json:"max_results"   env:"HERON_TOOLS_WEB_GLM_MAX_RESULTS"`
}
...
type BaiduSearchConfig struct {
	Enabled    bool   `json:"enabled"     env:"HERON_TOOLS_WEB_BAIDU_ENABLED"`
	BaseURL    string `json:"base_url"    env:"HERON_TOOLS_WEB_BAIDU_BASE_URL"`
	MaxResults int    `json:"max_results" env:"HERON_TOOLS_WEB_BAIDU_MAX_RESULTS"`
	apiKey     string
	secDirty   bool
}
...
type WebToolsConfig struct {
	ToolConfig  `                  envPrefix:"HERON_TOOLS_WEB_"`
	...
	PreferNative bool `json:"prefer_native" env:"HERON_TOOLS_WEB_PREFER_NATIVE"`
	// Proxy is an optional proxy URL for web tools (http/https/socks5/socks5h).
	// For authenticated proxies, prefer HTTP_PROXY/HTTPS_PROXY env vars instead of embedding credentials in config.
	Proxy                string              `json:"proxy,omitempty"                  env:"HERON_TOOLS_WEB_PROXY"`
	FetchLimitBytes      int64               `json:"fetch_limit_bytes,omitempty"      env:"HERON_TOOLS_WEB_FETCH_LIMIT_BYTES"`
	Format               string              `json:"format,omitempty"                 env:"HERON_TOOLS_WEB_FORMAT"`
	PrivateHostWhitelist FlexibleStringSlice `json:"private_host_whitelist,omitempty" env:"HERON_TOOLS_WEB_PRIVATE_HOST_WHITELIST"`
}

type CronToolsConfig struct {
	ToolConfig         `     envPrefix:"HERON_TOOLS_CRON_"`
	ExecTimeoutMinutes int  `                                 env:"HERON_TOOLS_CRON_EXEC_TIMEOUT_MINUTES" json:"exec_timeout_minutes"` // 0 means no timeout
	AllowCommand       bool `                                 env:"HERON_TOOLS_CRON_ALLOW_COMMAND"        json:"allow_command"`
}

type ExecConfig struct {
	ToolConfig          `         envPrefix:"HERON_TOOLS_EXEC_"`
	EnableDenyPatterns  bool     `                                 env:"HERON_TOOLS_EXEC_ENABLE_DENY_PATTERNS"  json:"enable_deny_patterns"`
	AllowRemote         bool     `                                 env:"HERON_TOOLS_EXEC_ALLOW_REMOTE"          json:"allow_remote"`
	CustomDenyPatterns  []string `                                 env:"HERON_TOOLS_EXEC_CUSTOM_DENY_PATTERNS"  json:"custom_deny_patterns"`
	CustomAllowPatterns []string `                                 env:"HERON_TOOLS_EXEC_CUSTOM_ALLOW_PATTERNS" json:"custom_allow_patterns"`
	TimeoutSeconds      int      `                                 env:"HERON_TOOLS_EXEC_TIMEOUT_SECONDS"       json:"timeout_seconds"` // 0 means use default (60s)
}

type SkillsToolsConfig struct {
	ToolConfig            `                       envPrefix:"HERON_TOOLS_SKILLS_"`
	Registries            SkillsRegistriesConfig `                                   json:"registries"`
	Github                SkillsGithubConfig     `                                   json:"github"`
	MaxConcurrentSearches int                    `                                   json:"max_concurrent_searches" env:"HERON_TOOLS_SKILLS_MAX_CONCURRENT_SEARCHES"`
	SearchCache           SearchCacheConfig      `                                   json:"search_cache"`
}

type MediaCleanupConfig struct {
	ToolConfig `    envPrefix:"HERON_MEDIA_CLEANUP_"`
	MaxAge     int `                                    env:"HERON_MEDIA_CLEANUP_MAX_AGE"  json:"max_age_minutes"`
	Interval   int `                                    env:"HERON_MEDIA_CLEANUP_INTERVAL" json:"interval_minutes"`
}
...
type ToolsConfig struct {
	AllowReadPaths  []string `json:"allow_read_paths"  env:"HERON_TOOLS_ALLOW_READ_PATHS"`
	AllowWritePaths []string `json:"allow_write_paths" env:"HERON_TOOLS_ALLOW_WRITE_PATHS"`
	...
	FilterSensitiveData bool `json:"filter_sensitive_data" env:"HERON_TOOLS_FILTER_SENSITIVE_DATA"`
	...
	// Default: 8
	FilterMinLength int                `json:"filter_min_length" env:"HERON_TOOLS_FILTER_MIN_LENGTH"`
	...
	AppendFile      ToolConfig         `json:"append_file"                                              envPrefix:"HERON_TOOLS_APPEND_FILE_"`
	EditFile        ToolConfig         `json:"edit_file"                                                envPrefix:"HERON_TOOLS_EDIT_FILE_"`
	FindSkills      ToolConfig         `json:"find_skills"                                              envPrefix:"HERON_TOOLS_FIND_SKILLS_"`
	I2C             ToolConfig         `json:"i2c"                                                      envPrefix:"HERON_TOOLS_I2C_"`
	InstallSkill    ToolConfig         `json:"install_skill"                                            envPrefix:"HERON_TOOLS_INSTALL_SKILL_"`
	ListDir         ToolConfig         `json:"list_dir"                                                 envPrefix:"HERON_TOOLS_LIST_DIR_"`
	Message         ToolConfig         `json:"message"                                                  envPrefix:"HERON_TOOLS_MESSAGE_"`
	ReadFile        ReadFileToolConfig `json:"read_file"                                                envPrefix:"HERON_TOOLS_READ_FILE_"`
	SendFile        ToolConfig         `json:"send_file"                                                envPrefix:"HERON_TOOLS_SEND_FILE_"`
	Spawn           ToolConfig         `json:"spawn"                                                    envPrefix:"HERON_TOOLS_SPAWN_"`
	SpawnStatus     ToolConfig         `json:"spawn_status"                                             envPrefix:"HERON_TOOLS_SPAWN_STATUS_"`
	SPI             ToolConfig         `json:"spi"                                                      envPrefix:"HERON_TOOLS_SPI_"`
	Subagent        ToolConfig         `json:"subagent"                                                 envPrefix:"HERON_TOOLS_SUBAGENT_"`
	WebFetch        ToolConfig         `json:"web_fetch"                                                envPrefix:"HERON_TOOLS_WEB_FETCH_"`
	WriteFile       ToolConfig         `json:"write_file"                                               envPrefix:"HERON_TOOLS_WRITE_FILE_"`
}
...
type SearchCacheConfig struct {
	MaxSize    int `json:"max_size"    env:"HERON_SKILLS_SEARCH_CACHE_MAX_SIZE"`
	TTLSeconds int `json:"ttl_seconds" env:"HERON_SKILLS_SEARCH_CACHE_TTL_SECONDS"`
}
...
type SkillsGithubConfig struct {
	token    string
	secDirty bool
	Proxy    string `json:"proxy,omitempty" env:"HERON_TOOLS_SKILLS_GITHUB_PROXY"`
}
...
type ClawHubRegistryConfig struct {
	Enabled         bool   `json:"enabled"           env:"HERON_SKILLS_REGISTRIES_CLAWHUB_ENABLED"`
	BaseURL         string `json:"base_url"          env:"HERON_SKILLS_REGISTRIES_CLAWHUB_BASE_URL"`
	authToken       string
	secDirty        bool
	SearchPath      string `json:"search_path"       env:"HERON_SKILLS_REGISTRIES_CLAWHUB_SEARCH_PATH"`
	SkillsPath      string `json:"skills_path"       env:"HERON_SKILLS_REGISTRIES_CLAWHUB_SKILLS_PATH"`
	DownloadPath    string `json:"download_path"     env:"HERON_SKILLS_REGISTRIES_CLAWHUB_DOWNLOAD_PATH"`
	Timeout         int    `json:"timeout"           env:"HERON_SKILLS_REGISTRIES_CLAWHUB_TIMEOUT"`
	MaxZipSize      int    `json:"max_zip_size"      env:"HERON_SKILLS_REGISTRIES_CLAWHUB_MAX_ZIP_SIZE"`
	MaxResponseSize int    `json:"max_response_size" env:"HERON_SKILLS_REGISTRIES_CLAWHUB_MAX_RESPONSE_SIZE"`
}
...
type MCPConfig struct {
	ToolConfig `                    envPrefix:"HERON_TOOLS_MCP_"`
...
func LoadConfig(path string) (*Config, error) {
			...
			for _, k := range m.apiKeys {
				if k != "" && !strings.HasPrefix(k, "enc://") && !strings.HasPrefix(k, "file://") {
					fmt.Fprintf(os.Stderr,
						"heron: warning: model %q has a plaintext api_key; call SaveConfig to encrypt it\n",
						m.ModelName)
	...
	if cfg.Agents.Defaults.Workspace == "" {
		...
		if heronHome := os.Getenv(EnvHome); heronHome != "" {
			homePath = heronHome
		} else if homePath != "" {
			homePath = filepath.Join(homePath, pkg.DefaultHeronHome)
		}
