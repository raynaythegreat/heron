...
// HERON_SSH_KEY_PATH to its path for the duration of the test. This is required
// whenever a test exercises encryption/decryption via credential.Encrypt or SaveConfig.
func mustSetupSSHKey(t *testing.T) {
	t.Helper()
	keyPath := filepath.Join(t.TempDir(), "heron_ed25519.key")
	if err := credential.GenerateSSHKey(keyPath); err != nil {
		t.Fatalf("mustSetupSSHKey: %v", err)
	}
	t.Setenv("HERON_SSH_KEY_PATH", keyPath)
}
  ...
  "hooks": {
    "processes": {
      "review-gate": {
        "enabled": true,
        "transport": "stdio",
        "command": ["uvx", "heron-hook-reviewer"],
        "dir": "/tmp/hooks",
        "env": {
          "HOOK_MODE": "rewrite"
        },
...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	cfg := DefaultConfig()
	want := filepath.Join(fakeHome, ".heron", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	t.Setenv("HERON_HOME", "/custom/heron/home")

	cfg := DefaultConfig()
	want := filepath.Join("/custom/heron/home", "workspace")

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with HERON_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
...
`
	...
	t.Setenv("HERON_KEY_PASSPHRASE", "test-passphrase")
	t.Setenv("HERON_SSH_KEY_PATH", "")
...
// unchanged when HERON_KEY_PASSPHRASE is not set.
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when HERON_KEY_PASSPHRASE
