...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via HERON_KEY_PASSPHRASE
...
//  2. HERON_SSH_KEY_PATH env var
//  3. ~/.ssh/heron_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "HERON_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading HERON_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "HERON_SSH_KEY_PATH"

// heronHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const heronHome = "HERON_HOME"

const (
	fileScheme = "file://"
	encScheme  = "enc://"
	hkdfInfo   = "heron-credential-v1"
	saltLen    = 16
	nonceLen   = 12
	keyLen     = 32
)
...
//
// passphrase is required (HERON_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// HERON_SSH_KEY_PATH env var or ~/.ssh/heron_ed25519.key.
...
//   - exact match with HERON_SSH_KEY_PATH env var
//   - within the HERON_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with HERON_SSH_KEY_PATH.
	...
	// Within HERON_HOME.
	if picoHome := os.Getenv(heronHome); picoHome != "" {
...
// Final key: HKDF-SHA256(ikm, salt, info="heron-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set HERON_SSH_KEY_PATH or place key at ~/.ssh/heron_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (HERON_SSH_KEY_PATH, HERON_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. HERON_SSH_KEY_PATH env var
//  3. ~/.ssh/heron_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the heron-specific SSH key path if it exists.
