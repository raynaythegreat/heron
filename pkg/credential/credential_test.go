...
func TestResolve_EncKey_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	sshKeyPath := filepath.Join(dir, "heron_ed25519.key")
	...
	t.Setenv("HERON_SSH_KEY_PATH", sshKeyPath)
	...
	t.Setenv("HERON_KEY_PASSPHRASE", passphrase)
...
func TestResolve_EncKey_WithSSHKey(t *testing.T) {
	...
	// Set HERON_SSH_KEY_PATH before Encrypt so the path passes allowedSSHKeyPath validation.
	t.Setenv("HERON_KEY_PASSPHRASE", passphrase)
	t.Setenv("HERON_SSH_KEY_PATH", sshKeyPath)
...
func TestResolve_EncKey_NoPassphrase(t *testing.T) {
	...
	_, err = r.Resolve(enc)
	if err == nil {
		t.Fatal("expected error when HERON_KEY_PASSPHRASE is unset, got nil")
	}
}

func TestResolve_EncKey_BadCiphertext(t *testing.T) {
	t.Setenv("HERON_KEY_PASSPHRASE", "some-passphrase")
...
func TestResolve_EncKey_WrongPassphrase(t *testing.T) {
	...
	t.Setenv("HERON_KEY_PASSPHRASE", "wrong-passphrase")
...
func TestDeriveKey_SSHKeyNotFound(t *testing.T) {
	...
	t.Setenv("HERON_KEY_PASSPHRASE", "passphrase")
	t.Setenv("HERON_SSH_KEY_PATH", filepath.Join(dir, "nonexistent_key"))
...
// that are not under HERON_SSH_KEY_PATH, HERON_HOME, or ~/.ssh/.
